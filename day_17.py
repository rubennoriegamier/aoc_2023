...
def create_graph(heat_map: list[list[int]], *, min_dist: int, max_dist: int) -> Graph:
    ...
    for y in range(h):
        for x in range(w):
            for distance in range(1, min(x + 1, max_dist + 1)):
                vs.append((y, x, '>', distance))
            for distance in range(1, min(y + 1, max_dist + 1)):
                vs.append((y, x, 'v', distance))
            for distance in range(1, min(w - x, max_dist + 1)):
                vs.append((y, x, '<', distance))
            for distance in range(1, min(h - y, max_dist + 1)):
                vs.append((y, x, '^', distance))

    g = Graph(directed=True)
    g.add_vertices(vs)
    get_v = {v['name']: v for v in g.vs}
    tl = get_v[vs[0]]
    br = get_v[vs[1]]
    es = [(tl, get_v[0, 1, '>', 1]),
          (tl, get_v[1, 0, 'v', 1])]
